...
function retrieveContacts(){
  ...
  $.ajax({
    method: "POST",
    url: _domain + "retrieveBlockedContacts",
    data: credentials
  }).done(function(data) {
    console.log(data);
    if(data.code == 200){
...
function reinstateUser(index) {
  ...
  var dataObject = {
    "emailAddress" : _emailAddress,
    "token": _token,
    "unblockUserEmail" : _blockingEmail
  };

  $.ajax({
    method: "POST",
    url: _domain + "admin/reinstateAccount",
    data: dataObject
  }).done(function(data) {
    console.log(data);
    if(data.code == 200){
...
$("#listOfUsers").on("click", "li", function() {
  ...
  $(function() {
    $( "#dialog-confirm-reinstate" ).dialog({
      resizable: false,
      height:140,
      modal: true,
      buttons: {
        "Unblock": function() {
          reinstateUser(indexOfUser);
          $( this ).dialog( "close" );
        },
        Cancel: function() {
          $( this ).dialog( "close" );
